...
namespace DatabaseManager {
    ...
    public partial class DbForm : Form, IGuiCallback {
        ...
        public void OnAddTag(Tag tag) {
            ...
            if (listViewTags.FindItemWithText(tag.Id) != null) return;
        ...
        public void OnRemoveTag(Tag tag) {
            ...
            ListViewItem item = listViewTags.FindItemWithText(tag.Id);
            if (item != null) listViewTags.Items.Remove(item);
        }
        ...
        private void buttonRemoveAlarm_Click(object sender, EventArgs e) {
            if (listViewTags.SelectedItems.Count <= 0 || listViewAlarms.SelectedItems.Count <= 0) return;

            ListViewItem tagItem = listViewTags.SelectedItems[0];
            ListViewItem alarmItem = listViewAlarms.SelectedItems[0];

            var tag = (Tag) tagItem.Tag;
            var alarm = (Alarm) alarmItem.Tag;
            if (_proxy.RemoveInputTagAlarm(tag.Id, alarm.Id)) listViewAlarms.Items.Remove(alarmItem);
        }
