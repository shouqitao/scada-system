...
namespace DatabaseManager {
    ...
    public partial class TagForm : Form, IGuiCallback {
        ...
        public TagForm(Operation operation, ListViewItem clickedItem) {
            ...
            var factory =
                new DuplexChannelFactory<IDatabaseManager>(this, binding, new EndpointAddress(address));
        ...
        private void buttonDone_Click(object sender, EventArgs e) {
            ...
            else if (_operation == Operation.Update) DoneClickUpdateOperation();
        }

        private void DoneClickAddOperation() {
            if (!InputsFilledCorrectly()) return;

            Tag newTag = CreateTagFromInputs();
        ...
        private void DoneClickUpdateOperation() {
            ...
            Tag updatedTag = CreateTagFromInputs();
        ...
        private Tag CreateTagFromInputs() {
            switch (comboBoxTagType.SelectedItem.ToString()) {
                case "Analog Input":
                    return new AnalogInputTag(textBoxId.Text, textBoxDescription.Text, null,
                        textBoxAddress.Text,
                        Convert.ToInt32(textBoxScanTime.Text), new List<Alarm>(), checkBoxOnScan.Checked,
                        checkBoxAutoMode.Checked,
                        (FunctionType) Enum.Parse(typeof(FunctionType),
                            comboBoxFunction.SelectedItem.ToString()),
                        Convert.ToDouble(textBoxLowLimit.Text), Convert.ToDouble(textBoxHighLimit.Text),
                        textBoxUnits.Text
                    );

                case "Analog Output":
                    return new AnalogOutputTag(textBoxId.Text, textBoxDescription.Text, null,
                        textBoxAddress.Text,
                ...
                case "Digital Input":
                    return new DigitalInputTag(textBoxId.Text, textBoxDescription.Text, null,
                        textBoxAddress.Text,
                        Convert.ToInt32(textBoxScanTime.Text), new List<Alarm>(), checkBoxOnScan.Checked,
                        checkBoxAutoMode.Checked, FunctionType.Digital);

                case "Digital Output":
                    return new DigitalOutputTag(textBoxId.Text, textBoxDescription.Text, null,
                        textBoxAddress.Text,
        ...
        private void SetInputsByTag(Tag tag) {
            ...
            else if (tag.GetType() == typeof(DigitalOutputTag)) comboBoxTagType.SelectedIndex = 3;
        ...
        private void PopulateInputsByTag(Tag tag) {
            ...
            if (tag.GetType() == typeof(AnalogInputTag)) {
                ...
                textBoxInitialValue.Text =
                    ((AnalogOutputTag) tag).InitialValue.ToString(CultureInfo.CurrentCulture);
                textBoxLowLimit.Text = ((AnalogOutputTag) tag).LowLimit.ToString(CultureInfo.CurrentCulture);
                textBoxHighLimit.Text =
                    ((AnalogOutputTag) tag).HighLimit.ToString(CultureInfo.CurrentCulture);
                ...
                textBoxInitialValue.Text =
                    ((DigitalOutputTag) tag).InitialValue.ToString(CultureInfo.CurrentCulture);
