...
namespace SCADACore {
    ...
    public class WcfServer : IDatabaseManager, IAlarm, ITrending {
        ...
        public bool AddTag(Tag tag) {
            lock (Locker) {
                if (_tags.ContainsKey(tag.Id) ||
                    _tags.Values.Any(otherTag => otherTag.Address == tag.Address)) return false;

                tag.Driver = _simulationDriver;
                if (tag is OutputTag) tag.SetValue(((OutputTag) tag).InitialValue);

                _tags[tag.Id] = tag;
                PublishTagAdded(tag);
                if (tag is InputTag && ((InputTag) tag).AutoMode) StartInputTagUpdatingThread(tag);
                return true;
            }
        }
        ...
        public bool RemoveTag(string tagId) {
            lock (Locker) {
                if (!_tags.ContainsKey(tagId)) return false;
        ...
        public bool AddInputTagAlarm(string tagId, Alarm alarm) {
            lock (Locker) {
                if (!_tags.ContainsKey(tagId) || !(_tags[tagId] is InputTag)) return false;

                Tag tag = _tags[tagId];
                var inputTag = (InputTag) tag;
                Alarm otherAlarm = inputTag.Alarms.FirstOrDefault(a => a.Id == alarm.Id);
                if (otherAlarm != null) return false;
        ...
        public bool RemoveInputTagAlarm(string tagId, string alarmId) {
            lock (Locker) {
                ...
                Tag tag = _tags[tagId];
                var inputTag = (InputTag) tag;
                Alarm alarm = inputTag.Alarms.FirstOrDefault(a => a.Id == alarmId);
        ...
        public bool UpdateTag(Tag updatedTag) {
            Tag originalTag = _tags.Values.FirstOrDefault(t => t.Id == updatedTag.Id);
            if (originalTag == null) return false;

            if (updatedTag is OutputTag) {
                double value = updatedTag.GetValue();
            ...
            if (updatedTag is InputTag) ((InputTag) updatedTag).Alarms = ((InputTag) originalTag).Alarms;
        ...
        public void SubscribeToTags() {
            lock (CallbacksLocker) {
                ...
                foreach (Tag tag in _tags.Values) PublishTagAdded(tag);
            }
        }
        ...
        public void PublishTagAdded(Tag tag) {
            lock (CallbacksLocker) {
                foreach (IGuiCallback guiCallback in _guiCallbacks) guiCallback?.OnAddTag(tag);
            }
        }

        public void PublishTagRemoved(Tag tag) {
            lock (CallbacksLocker) {
                foreach (IGuiCallback guiCallback in _guiCallbacks) guiCallback?.OnRemoveTag(tag);
            }
        }

        public void PublishTagUpdated(Tag tag) {
            lock (CallbacksLocker) {
                foreach (IGuiCallback guiCallback in _guiCallbacks) guiCallback?.OnUpdateTag(tag);
            }
        }
        ...
        private static void LogAlarmToFile(string alarmMessage) {
            lock (FileLocker) {
                using (StreamWriter writer = File.AppendText(ScadaConstants.AlarmsLogPath)) {
        ...
        private void DeserializeTags() {
            ...
            using (var reader = new StreamReader("scadaConfig.xml")) {
                ...
                lock (Locker) {
                    if (tagsList != null) _tags = tagsList.ToDictionary(tag => tag.Id);
        ...
        private static void AbortThreads() {
            foreach (Thread thread in _threads.Values) thread.Abort();
        }
