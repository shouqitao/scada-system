    ...
    public class WcfServer : IDatabaseManager, IAlarm, ITrending {
        ...
        public bool AddTag(Tag tag) {
            lock (Locker) {
                ...
                if (tag is OutputTag outputTag)
                    outputTag.SetValue(outputTag.InitialValue);

                _tags[tag.Id] = tag;
                PublishTagAdded(tag);
                if (tag is InputTag inputTag && inputTag.AutoMode)
                    StartInputTagUpdatingThread(inputTag);
                return true;
            }
        }
        ...
        public void SubscribeToTags() {
            lock (CallbacksLocker) {
                ...
                foreach (var tag in _tags.Values) PublishTagAdded(tag);
            }
        }
        ...
        private static void AbortThreads() {
            foreach (var thread in _threads.Values) thread.Abort();
        }
